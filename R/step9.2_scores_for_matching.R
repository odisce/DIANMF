...
match_pure_scores2 <- function(polarity, mz_prec, data_base, measured_spectra, mz_tol = NULL, idx = NA) {
  ...
  if( is.na(idx) ){
    idx_ref_spect <- data_base$index[Polarity == polarity & dplyr::between(mz, mz_prec - 0.05, mz_prec + 0.05), id]
  } else {
    idx_ref_spect <- idx
  }
