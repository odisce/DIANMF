test_that("Step 1 tests", {
  ...
  expect_true( all(c("mzml_path", "file_name", "creation_date", "class", "InjectionOrder") %in% colnames(mzml_dt)) )

  mzml_seq <- create_seq(mzml_dt)
  expect_equal( nrow(mzml_dt), nrow(mzml_seq) )
  expect_true( all(c("mzml_path", "class", "InjectionOrder") %in% colnames(mzml_seq)) )
...
#   s_idx = 1
#   
#   ms1_features <- copy(ms1_features_2[sample == s_idx, ])
...
# }
  ...
  # sample_idx <- 1
  # rt_range = c(280, 320)
  # msexp_idx <- xcms::filterFile(msexp, sample_idx)
  # msexp_idx_rt <- xcms::filterRt(msexp_idx, rt_range) %>% suppressMessages()
  # 
  # ms1_mixed_mat_nb1 <- build_XICs( peaks_dt = msexp, rawdt = msexp_idx_rt, method = c("all", "max")[2])
  # ms1_mixed_mat_nb2 <- build_XICs( peaks_dt = msexp, rawdt = , method = c("all", "max")[1])
  # 
