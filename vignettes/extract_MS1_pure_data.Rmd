...
The MS1_features object contains information for every identified peak as the peak itself, the pure spectra and elution profiles (or chromatograms) for every pure component in its mixed data (called W_ms1 and H_ms1 respectively), and some other helpful information gained while processing the peak. For example, the pure spectra corresponding to the peaks can be extracted using the following function.
...
To familiarize ourselves with the different functions in the *DIANMF* package, we will process one specific peak, the first.
